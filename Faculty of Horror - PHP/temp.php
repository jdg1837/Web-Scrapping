...
function extractFromPage($src)
{
    ...
    if(!empty($html)) {
        foreach($html->find('.name_directory_name_box') as $divClass) {
            ...
            $episode_links = array();
            // $top_list = $divClass->find('div ul');
            // foreach($top_list->find('li a') as $list_entry){
            foreach($divClass->find('div ul li a') as $list_entry){
...
// }
#Get one
$letter_index = extractFromPage("https://www.facultyofhorror.com/episode-index/?name_directory_startswith=S");
$index = array_merge($index, $letter_index);
